...
- When asked about hiring, interviewing, or rates, state that Kunal is open to Staff/Principal Backend and AI Infrastructure roles (hybrid in Bengaluru or remote), and encourage them to email kunalkumar1.dev@gmail.com.`;
...
He applies these principles directly to SIMD vector kernels, GPU memory virtualization, and high-throughput inference serving. Check out the **Academic & Theoretical Foundations** section on this page to explore his notes and takeaways!`;
    ...
    if (q.includes('hire') || q.includes('contact') || q.includes('email') || q.includes('resume') || q.includes('job') || q.includes('availab')) {
      return `Kunal is open to **Staff Backend** and **Principal AI Systems Engineer** roles (Bengaluru or remote). You can reach him directly at **kunalkumar1.dev@gmail.com** or open his verified Resume directly from the navigation bar!`;
    }
  ...
  // AI Chat Bot endpoint using Gemini 3.6 Flash
    ...
    try {
      ...
      for (let attempt = 0; attempt < 2; attempt += 1) {
        try {
          ...
          const isTransient = /503|UNAVAILABLE|429|RESOURCE_EXHAUSTED|timeout/i.test(message);
